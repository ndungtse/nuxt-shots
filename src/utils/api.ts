...
export const getCuratedPhotos = async (options?: FetchOptions) => {
  const response = await PHOTOS_API.get(`/curated?per_page=${options?.per_page ?? 10}&page=${options?.page ?? 1}`); //
  return response.data;
};
...
export const getCuratedVideos = async (options?: FetchOptions) => {
  const response = await VIDEO_API.get(`/popular?per_page=${options?.per_page ?? 10}&page=${options?.page ?? 1}`);
  return response.data;
};

export const searchVideos = async (query: string, options?: FetchOptions) => {
  const response = await VIDEO_API.get(`/search?query=${query}&per_page=${options?.per_page ?? 10}&page=${options?.page ?? 1}`);
  return response.data;
};
...
export const getVideoStream = async (
  ...
  const response = await VIDEO_API.get(`/videos/${id}/stream?format=${format}`);
  return response.data;
};
