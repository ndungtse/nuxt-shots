...
const navs = ref<{ name: string, path: string }[]>([{ name: "home", path: '/' }, { name: "videos", path: '/videos' }, { name: "favorites", path: '/favorites' }, { name: "about", path: '/about' }]);

const changeTheme = () => {
  const html = document.querySelector('html')
  if (html?.classList.contains('dark')) {
    console.log('contains', html.classList.value);
...
<template>
  <nav
    ...
    <div class="flex gap-x2 items-center">
      ...
      <!-- <label @click="changeTheme" class="swap swap-rotate">
        <input type="checkbox" />
